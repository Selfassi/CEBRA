...
```{r merge_with_industry_data, eval = TRUE}
        ...
        "Broadacre crops - Cereal crops - Barley for grain - Area (ha)" | agri_regions$Commodity == "Broadacre crops - Cereal crops - Wheat for grain - Area (ha)")
        ...
        fruit_regions <-filter(agri_regions, agri_regions$Commodity == a | agri_regions$Commodity == b | agri_regions$Commodity == c | agri_regions$Commodity == d)
        ...
        #Vegetables
        veg_regions <- filter(agri_regions, agri_regions$Commodity == "Vegetables for human consumption - All other - Area (ha)" )
...
```{r species_range_map, eval = FALSE}
        ...
        for (i in 1:length(species))
        {
                ...
                file<-paste(species[i],"_map",".png", sep="")
...
```{r fig.width=5, fig.height=5, eval = TRUE}
        ...
        img <- readPNG("Giant African snail_map.png")
        grid.raster(img)
...
```{r inputs_for_app, eval = TRUE}
        ...
        #tidy the dataframe
        colnames(input_app)<-c("Overlap_Flowers","Overlap_Crops","Overlap_Fruit","Overlap_Hay","Overlap_Livestock","Overlap_Veg","Overlap_Population_Numbers")
